...
import { scoreStats } from "@/lib/fantasy/scoring";
...
function layoutRoster(ids: string[], byId: Map<string, SlimPlayer>, week: number) {
  const pool = ids
    .map((id) => byId.get(id))
    .filter((p): p is SlimPlayer => !!p)
    .sort(
      (a, b) =>
        scoreStats(projectedStats(b, week), { rec: 0 } as never) -
        scoreStats(projectedStats(a, week), { rec: 0 } as never),
    );
...
function TeamImporter({
  ...
  const setRoster = (ids: string[]) => {
    ...
    updateLeague((l) => ({
      ...l,
      teams: l.teams.map((t) =>
        t.id === team.id ? { ...t, ...layoutRoster(unique, byId, week) } : t,
      ),
    }));
  };
